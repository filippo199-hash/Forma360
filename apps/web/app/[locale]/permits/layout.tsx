...
/**
 * Permit to Work shell (FreeHS B3). Brand-gated at the routing layer
 * (ADR 0010) — a brand without the module 404s the whole subtree — and
 * wraps the pages in `PermissionsProvider` so they can read
 * `useHasPermission('permits.*')`. The server still enforces both gates
 * regardless.
 */
export default async function PermitsLayout({
  ...
  if (!brandHasModule(activeBrand.id, 'permits')) {
    notFound();
  }
