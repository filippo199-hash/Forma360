...
/**
 * Fire Safety shell (FreeHS B3). Brand-gated at the routing layer
 * (ADR 0010) — a brand without the module 404s the whole subtree — and
 * wraps the pages in `PermissionsProvider` so they can read
 * `useHasPermission('fireSafety.*')`. The server still enforces both
 * gates regardless.
 */
export default async function FireSafetyLayout({
  ...
  if (!brandHasModule(activeBrand.id, 'fireSafety')) {
    notFound();
  }
