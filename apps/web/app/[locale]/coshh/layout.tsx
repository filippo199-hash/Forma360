...
/**
 * COSHH shell (FreeHS B2). Brand-gated at the routing layer (ADR 0010) —
 * a brand without the module 404s the whole subtree — and wraps the pages
 * in `PermissionsProvider` so they can read `useHasPermission('coshh.*')`.
 * The server still enforces both gates regardless.
 */
export default async function CoshhLayout({
  children,
  params,
}: {
  children: ReactNode;
