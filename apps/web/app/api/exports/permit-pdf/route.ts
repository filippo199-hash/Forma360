/**
 * Session-gated permit PDF download (HSE review PW-6 — the postable
 * copy for the job face and the fixed record for the audit bundle).
 * Renders (or re-uses) the PDF in R2 and 302s to a short-lived signed
 * URL, mirroring /api/exports/risk-assessment-pdf.
 */
...
export async function GET(req: Request): Promise<Response> {
  ...
  let rendered: Awaited<ReturnType<typeof caller.permits.renderPdf>>;
  try {
    rendered = await caller.permits.renderPdf({ permitId });
  } catch (err) {
    const code =
