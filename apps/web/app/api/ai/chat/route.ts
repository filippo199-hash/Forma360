...
const TOOLS: Anthropic.Tool[] = [
  {
    ...
    description: 'List recent inspections for this company. Use to answer questions about inspection status, history, or activity.',
    input_schema: {
      type: 'object' as const,
      properties: {
        limit: { type: 'number', description: 'Max results (default 10, max 50)' },
        status: { type: 'string', enum: ['in_progress', 'submitted', 'completed', 'rejected'], description: 'Filter by status' },
      },
      required: [],
    },
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        limit: { type: 'number', description: 'Max results (default 10, max 50)' },
        status: { type: 'string', enum: ['open', 'investigation', 'closed'], description: 'Filter by status' },
      },
      required: [],
    },
  },
...
type ToolName = 'list_inspections' | 'list_issues' | 'list_actions' | 'list_assets' | 'list_headsup' | 'list_documents' | 'list_schedules';
...
export async function POST(request: Request) {
  ...
  void (async () => {
    try {
      ...
      await writer.write(sseChunk({ type: 'conversation', conversationId, isNew }));
      ...
      while (true) {
        ...
        sdkStream.on('text', (text) => {
          ...
          void writer.write(sseChunk({ type: 'text', delta: text }));
        });
        ...
        for (const toolBlock of toolUseBlocks) {
          void writer.write(sseChunk({ type: 'tool_call', toolName: toolBlock.name }));
          try {
