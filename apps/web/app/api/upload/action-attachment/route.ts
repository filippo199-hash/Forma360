/**
 * Action attachment upload endpoint.
 *
 * Mirrors apps/web/app/api/upload/observation-attachment/route.ts but
 * targets the `actions` module. Accepts a multipart upload, writes the
 ...
 *
 * The metadata row in `action_attachments` is created by the caller via
 * the `actions.attachments.create` tRPC mutation — this route is the
 * blob-only half of that flow.
 *
 * Storage key layout:
 *   <tenantId>/actions/<actionId>/<filename>
 *
 * Auth: session-required + `actions.view`. Tenant scope is enforced by
 * the session.
 */
...
export async function POST(req: Request): Promise<Response> {
  ...
  if (!perms.includes('actions.view')) {
  ...
  if (actionId.length !== 26 || !(file instanceof File)) {
  ...
  if (env.NODE_ENV === 'production') {
    try {
      ...
      if (!res.ok) {
        return await storageFailed(ctx.logger, 'action-attachment', key, res);
      }
    } catch (err) {
      ctx.logger.error({ err }, '[action-attachment] R2 PUT threw');
  ...
  if (env.NODE_ENV === 'production') {
    try {
      ...
      ctx.logger.warn({ err, key }, '[action-attachment] signed URL failed');
    }
  }
