...
import { docsLibrary, GETTING_STARTED_GROUP, guides, type Guide } from '../../content/guides';
import { MARKETING_PAGES } from '../../content/site';
import { MODULE_ICONS } from './module-icon';

/**
 * The `/docs` library index: a search box over the whole library, then
 * getting-started first and every module's guides grouped under its
 * category — a library to consult, not a wall of links. Content comes
 * from `content/guides` (brand-filtered); a client component only for
 * the search filter, which runs over the same static data the server
 * would have rendered.
 */
  ...
  const results = useMemo(() => {
    ...
    return guides().filter(
      (guide) =>
        ...
        guide.summary.toLowerCase().includes(trimmed) ||
        guide.area.replace(/-/g, ' ').includes(trimmed),
    );
  }, [trimmed]);
      ...
      <header className="mx-auto max-w-2xl text-center">
        ...
        <div className="relative mx-auto mt-6 max-w-md">
          ...
          <input
            ...
            className="h-11 w-full rounded-xl border bg-card pl-10 pr-10 text-sm outline-none transition-colors placeholder:text-muted-foreground focus:border-brand/50 focus:ring-2 focus:ring-brand/20"
          />
          {query !== '' ? (
            <button
              type="button"
              onClick={() => setQuery('')}
              aria-label={copy.searchPlaceholder}
              className="absolute right-2.5 top-1/2 flex h-6 w-6 -translate-y-1/2 items-center justify-center rounded-md text-muted-foreground transition-colors hover:text-foreground"
            >
          ...
          {categories.map(({ category, modules }) => (
            <section key={category.key} className="mt-14">
              ...
              <div className="mt-4 grid gap-4 lg:grid-cols-2">
                {modules.map(({ module, guides: moduleGuides }) => {
...
function GuideCard({ locale, guide, minutes }: { locale: string; guide: Guide; minutes: string }) {
  return (
    <Link
      href={`/${locale}/docs/${guide.slug}`}
      className="group flex items-start justify-between gap-4 rounded-2xl border bg-card p-5 transition-all hover:-translate-y-0.5 hover:border-brand/40 hover:shadow-lg hover:shadow-brand/5"
    >
      <div>
        <h3 className="text-[15px] font-semibold tracking-tight">{guide.title}</h3>
        <p className="mt-1.5 line-clamp-2 text-sm leading-relaxed text-muted-foreground">
          {guide.summary}
        </p>
