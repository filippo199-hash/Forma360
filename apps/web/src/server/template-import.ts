...
- For multipleChoice, use the answer options the document implies (e.g. Pass/Fail/N/A, OK/Defect, Compliant/Non-compliant). Flag and colour the unsafe/failing option red. Reuse the EXACT same options across every question that shares a scale so they collapse into one shared response set — keep the number of distinct response sets to a minimum.
