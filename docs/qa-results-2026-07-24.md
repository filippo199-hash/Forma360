...
| **B4 (CONFIRMED, not yet fixed)** | **S2 (core feature broken)** | Inspections / conduct | A1 (Mandy Manager) | conduct → answer "Site conducted" → submit | Selecting the "Site conducted" site should set `inspection.siteId` so the report shows the site and site-filtering finds the inspection | The site answer is saved into `inspection.responses` (keyed by the question id) but **`inspection.siteId` / `siteName` stay null** after a clean submit. Report renders the site from `siteName` (report/page.tsx:419 `if (item.type==='site') titleResponses[item.id]=inspectionMeta.siteName??''`) → shows "Site conducted —". Every conducted inspection is then missing from `?site=` filters, Sites-overview linked inspections, and the `{site}` title token. Root cause: conduct-shell tracks `siteId` locally (line 805/1104) and feeds it to the raise-action payload (line 910) but never calls `inspections.setSite`; no server-side mirror of the `type:'site'` response into `inspection.siteId`. Proposed fix: mirror the `site`-type response → `inspection.siteId` in `saveProgress`/`submit` (server-side, robust), and resolve `siteName` for the report. | inspection 01KY9W8AHV2MWXPRS5DTZAMG42: responses has Manchester id, siteId=null; report ss_1211gelkd shows "—" | 1) conduct any template with a "Site conducted" question 2) select a site 3) submit 4) open report → "Site conducted —"; GET inspections.get → siteId null |
