...
| ID | Sev | Fix | Where |
| --- | --- | --- | --- |
| UXW56-01 | **S2** | The site question is now **requirable**: `isResponseRequirable` accepted every answer type that round-trips through `responses` except `site`, so a title-page site marked required never blocked submit. A completed walk shipped with `site_id` NULL — a blank Site on the printed report, invisible to every site-scoped view and compliance card, with nobody told. The server-side mirror into `inspection.siteId` already existed; only the gate was missing. The finding's second claim — that the selection itself failed to persist — was **instrument error** (the driver could not drive a Radix Select) and is corrected in the findings doc; the app saves the pick immediately, verified through the real control. Pinned by a new `conduct-state.test.ts` case (unanswered blocks, answered clears). | `inspections/conduct-state.ts` (+`.test.ts`) |
