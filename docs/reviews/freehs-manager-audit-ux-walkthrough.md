...
| UXW56-03 | E-words | S3 | Site page, compliance cards | Every compliance card carries its count ("Risk assessments 0", "Permits 1", "Observations 1") **except Fire safety, which shows no figure at all** — on the one module currently holding a red "Failed — awaiting re-test" state. The card most deserving a number (or the red chip itself) is the only silent one. |
