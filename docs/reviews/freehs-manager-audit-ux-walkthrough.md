...
| --- | --- | --- | --- | --- |
| UXW56-01 | E-act | **S2** | Conduct wizard → submit; report | The site-walk inspection **completed with its required Site empty** (`site_id` NULL in the record): the conduct's title page marks "Site*" required, and **submit enforced nothing** — `isResponseRequirable` accepted every answer type that round-trips through `responses` except `site`, so the gate skipped it. Consequence chain: the PDF prints a blank "Site", the site page's compliance views never see the walk, and nobody is told. *(Corrected at triage: this row originally also claimed the site selection "did not persist — autosave raced the page change". That was instrument error — the driver's `selectByLabel` cannot drive a Radix Select, so the pick never happened. Picking through the real control saves immediately. The app was accused of losing data it never lost; the harness now falls back to the listbox pattern so the two cannot be confused again.)* |
