...
export async function generateMaintenanceAction(
  ...
  // Resolve the owning program name so the activity log + auto badge can
  // say which program generated the action (best effort).
  const progRows = await db
    ...
    .limit(1);
  const programName = (progRows[0]?.name as string | undefined) ?? null;
  ...
  await db
    ...
    .onConflictDoNothing();
  // Record a `created` activity flagged `auto` so the timeline shows the
  // action was system-generated by the maintenance program rather than
  // raised by a person. Mirrors the manual-create activity write in the
  // actions router (inlined here to avoid a circular import).
